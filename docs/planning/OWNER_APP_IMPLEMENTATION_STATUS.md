...
| 발표 데모 모드 | 구현 | 실제 로그인과 숨김 진입 후 홈/메뉴/상품/수확/발주/신선도/배송/반품을 API 호출 없이 자동 시연 |
...
| 발표 데모 모드 | 95% | 실제 로그인 후 홈부터 반품 승인까지 발표용 자동 시연, 단계별 타이머, 펄스 터치 강조, 메뉴 글자색/버튼 강조, 일시정지/이전/다음 | 로컬 화면 전용 | API 호출 없이 재생해 401/세션 상태와 무관 | 실제 녹화는 로그인과 진입만 사람이 수행 |
