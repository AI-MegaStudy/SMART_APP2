...
| API/DB 실제 연결 | 91% | 계정, 상품, 농장, 수확 예측, 슬롯, 주문/예약, 발주, 신선도 DL/저장, 배송, 반품, 프로필 API 연결 |
| fallback 축소 | 90% | API 정상 응답 시 실제 DB를 우선 사용. 서버 장애/세션 만료/외부 모델 부재/플랫폼 제약 시에도 점주 업무 화면은 발표용 보조 흐름으로 유지 |
| 발표 완성도 | 95% | 개발자용 더미 문구 제거, ML 결과 카드/차트 수치, DL 신선도 결과, 갤러리 UX, 상태 처리, 선택형 입력 보강 |
...
현재 fallback 정책은 `API 정상 응답 > 실제 DB 데이터 표시`, `외부 DL 정상 응답 > 실제 분석값 표시/저장`, `API 실패/세션 만료/외부 연동 장애 > 사용자에게 자연스러운 보조 흐름 표시`이다. 발표 중 끊기면 안 되는 점주 업무 화면은 상품/농장/프로필/수확/신선도/배송/발주/반품까지 로컬 보조 데이터 또는 화면상 완료 처리로 이어진다.
...
| O-005 ML 예측 | 농장/상품/환경값/과거 수확량 입력 후 결과 저장 | Done | 수확 예측 화면에 입력값 selector/stepper, `예측에 반영된 기준`, 차트 kg 수치, 권장 예약량/판매가/`1,000㎡ 기준 수확량` 표시. `POST /owner/ml/predictions` 저장 smoke 통과. 세션/API 실패 시 발표용 양광/부사 예측 fallback |
...
| O-010 신선도 검사 | 이미지 선택/분석/점주 판정 저장 | Done | 갤러리 선택 버튼/카드 탭 연결, 외부 ngrok DL multipart 분석 우선 호출, 추천등급/신선도/색상/형태/멍 확률/판정 표시, DL 실패 시 백엔드 분석/앱 보조 판정 전환, `POST /owner/quality-inspections`에 실제 분석값 저장 |
...
- O-010은 외부 DL ngrok 응답을 앱 화면에 표시하고 저장 payload에 반영하도록 보강했다. ngrok/Kaggle 세션이 없으면 백엔드 분석과 앱 보조 판정으로 자동 전환한다.
